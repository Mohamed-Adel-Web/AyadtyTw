...
export default function EditDialog({
  ...
  const t = useTranslations('Dashboard.role.dialog'); // Using next-intl hook for translations
  
  const { register, formState, handleSubmit, reset } = useForm<Role>();
  const { mutate, isSuccess, isPending } = useEditData<Role>(
    rolesUrl,
    role?.id,
    "editRole",
    "allRole",
        ...
        <Accordion type="single" collapsible className="w-full mt-4">
          {sections.map((section) => (
            <AccordionItem key={section} value={section}>
              <AccordionTrigger>{t(`sections.${section}`)}</AccordionTrigger> {/* Use dynamic key */}
              <AccordionContent>
                      ...
                      <div className="inline-flex items-center">
                        ...
                        >
                          <input
                            ...
                            id={`${section}-${action}`}
                            {...register(`permissions.${section}.${action}`)}
                          />
